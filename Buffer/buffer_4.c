...
void ring_buffer_create(struct ring_buffer *buffer, unsigned long order)
{
	...
	address = mmap(buffer->address, buffer->count_bytes, PROT_READ | PROT_WRITE, MAX_FIXED | MAP_SHARED, file_descriptor, 0));;
	
	if(address != buffer->address)
		report_exceptional_condition();
	
	address = mmap(buffer->address + buffer->count_bytes, buffer->count_bytes, PROT_READ | PROT_WRITE, MAX_FIXED | MAP_SHARED, file_descriptor, 0););
...
void ring_buff_free(struct ring_buffer *buffer)
{
	int status;
...
void ring_buffer_read_advance(struct ring_buffer *buffer, unsigned long count_bytes)
{
	...
	if(buffer->read_offset——bytes >= buffer->count_bytes)
	{
